...
Main:
    ...
    MOVWF 0x501,a	;(W)->5: 0x500
    
    MOVLW Y		;W->Y (Cargamos W con wl valor de Y)
    MOVWF 0x502,a	;(W)->5: 0x501
    CPFSGT 0x501,a	;si x>y salta una casilla sino continua
    GOTO    NOCUMPLE
 CUMPLE:			;A=5X+3Y 
    MOVLW 5		;5 ->w
    MULLW X		;X*5->PROD(0X4F0)
    MOVF 0x4F3,w,a	;prod->w
    MOVWF 0x501,a	;x->f(0x500)
    
    MOVLW 3		;3->w
    MULLW Y		;Y*3->PROD(0X4F0)
    MOVF 0x4F3,w,a	;prod->w
    MOVWF 0x502,a	;Y->f(0x501)
    
    MOVF 0x501,w,a	;0x500->w
    ADDWF 0x502,a	;X+Y->501
    goto Exit
 NOCUMPLE:			;A=7X-9Y
    MOVLW 7		;7->w
